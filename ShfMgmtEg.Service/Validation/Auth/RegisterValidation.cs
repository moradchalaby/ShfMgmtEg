...
public class RegisterValidation : AbstractValidator<RegisterUser>
{
    ...
    public RegisterValidation(IUserService userService)
    {
        ...
        RuleFor(x => x.UserName).NotEmpty().MinimumLength(3).MustAsync(BeUniqueUser)
            .WithMessage("Fartklı bir kullanıcı adı seçiniz.");
        RuleFor(x => x.Email).NotEmpty().WithMessage("Email Boş Olamaz.").EmailAddress()
            .WithMessage("Email formatına uygun değil").MustAsync(BeUniqueUser).WithMessage("Email sistemde kayıtlı.");
        RuleFor(x => x.PhoneNumber).NotEmpty().WithMessage("Telefon Numarası Boş Olamaz.").MustAsync(BeUniqueUser)
            .WithMessage("Telefon numarası sistemde kayıtlı.");
